...
function acquireLock(projectId) {
  ...
  try {
    if (fs.existsSync(lockFile)) {
      const lockedPid = parseInt(fs.readFileSync(lockFile, 'utf8').trim(), 10);
      if (!isNaN(lockedPid)) {
        try {
          process.kill(lockedPid, 0); // throws if PID is dead
          return false;               // process alive → lock valid
        } catch (_) {
          log(projectId, `lock stale (PID ${lockedPid} muerto) — liberando`);
        }
      }
