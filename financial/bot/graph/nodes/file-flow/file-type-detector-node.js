...
async function fileTypeDetectorNode(state) {
  ...
  if (ext === 'xlsx' || ext === 'xls' || mimeType.includes('spreadsheet') || mimeType.includes('excel')) {
    try {
      ...
      if (xlsxResult?.tipo === 'cuadro_retorno' && xlsxResult.filas?.length > 0) {
        return { inputBuffer: buffer, cuadroRetorno: xlsxResult, nextAction: 'cuadro_retorno' };
      }
    } catch (e) {
  ...
  if (mimeType.startsWith('image/') && process.env.GOOGLE_API_KEY) {
    ...
    try {
      ...
      if (cuadro?.tipo === 'cuadro_retorno' && cuadro.filas?.length > 0) {
        return { inputBuffer: buffer, cuadroRetorno: cuadro, nextAction: 'cuadro_retorno' };
      }
    } catch (e) {
  ...
  if (detected.monto_total && (detected.tipo === 'comprobante' || detected.tipo === 'factura')) {
    return { inputBuffer: buffer, detectedFile: detected, nextAction: 'comprobante' };
  }

  if (detected.datos_bancarios?.length > 0) {
    return { inputBuffer: buffer, detectedFile: detected, nextAction: 'banking_extraction' };
  }
