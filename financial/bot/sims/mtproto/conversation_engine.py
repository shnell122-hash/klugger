...
AMBIENT_CHATS = [
    ...
    {"id": "comprobante_confirmado", "turns": [
...
def gen_op_scenario(acct: str, tipo: str, amount: int, tier: int) -> dict:
    ...
    return {
        ...
        "account": acct, "messages": messages, "expected": expected,
    }
...
def gen_scenarios_fin_mes(tier: int, active_accounts: set) -> list[dict]:
    ...
    for amount in amounts_drain:
        ...
        scenarios.append({"type": "chat", "tier": tier, "id": f"post_drain_{amount}", "turns": pick(CHAT_POST_OP)})

    scenarios.append({
        "type": "bot", "tier": tier, "id": "saldo_bajo_query",
    ...
    scenarios.append({
        "type": "chat", "tier": tier, "id": "kevin_propone_financiar",
        "turns": [
            {"account": "kevin", "message": "German, el saldo está muy bajo. ¿Quieres que anticipe yo las últimas?"},
            {"account": "gv",    "delay": 7, "message": "Sí Christian, son dos días para el cierre del mes. Adelanta las dos últimas y ya"},
    ...
    scenarios.append({
        "type": "chat", "tier": tier, "id": "cierre_mes_aviso",
        "turns": [
    ...
    scenarios.append({
        "type": "bot", "tier": tier, "id": "reintegro_factura",
        "account": reintegro_sender, "asset": ("comprobante", tier),
        "mode": "asistente", "expected": ["Saldo", "$"],
    })
    ...
    for amount in [random.choice([50_000, 75_000]) for _ in range(2)]:
        ...
        scenarios.append({"type": "chat", "tier": tier, "id": f"post_cierre_{amount}", "turns": pick(CHAT_POST_OP)})
...
def gen_scenarios(tier: int, active_accounts: set = None) -> list[dict]:
    ...
    for chat in random.sample(AMBIENT_CHATS, n_ambient):
        if any(t.get("account") in active_accounts for t in chat["turns"]):
            scenarios.append({"type": "chat", "tier": 1, "id": f"ambient_{chat['id']}", "turns": chat["turns"]})
    ...
    scenarios.append({
        "type": "bot", "tier": tier, "id": f"cuadro_png_{sender}", "account": sender,
        "asset": ("cuadro_png", tier), "mode": "asistente",
    ...
    scenarios.append({
        "type": "bot", "tier": tier, "id": f"clabe_{clabe_sender}", "account": clabe_sender,
        "messages": [clabe], "mode": "asistente",
    ...
    scenarios.append({
        "type": "bot", "tier": tier, "id": f"comprobante_{comp_sender}", "account": comp_sender,
        "asset": ("comprobante", tier), "mode": "asistente", "expected": ["Saldo", "$"],
    })

    for acct in sorted(active_accounts):
        frase = pick(FRASES_SALDO)
        scenarios.append({"type": "bot", "tier": tier, "id": f"saldo_{acct}", "account": acct,
                          "messages": [frase], "expected": ["saldo", "$"]})
    ...
    if tier >= 2:
        ...
        scenarios.append({
            "type": "bot", "tier": tier, "id": f"cuadro_xlsx_{xlsx_sender}", "account": xlsx_sender,
            "asset": ("cuadro_xlsx", tier), "mode": "asistente",
            "verify_db": "SELECT COUNT(*) FROM fin_operations WHERE created_at > NOW() - INTERVAL 180 SECOND",
            "expected": ["Saldo", "$"],
        })
        for acct in sorted(active_accounts):
        ...
        scenarios.append({"type": "chat", "tier": tier, "id": "ambient_t2_mid",
    ...
    if tier >= 3:
        ...
        for err_amount in random.sample(AMOUNTS_ERR, 2):
            scenarios.append({
                "type": "bot", "tier": tier,
                "id": f"monto_invalido_{err_amount}",
                "account": "gv" if "gv" in active_accounts else pick(sorted(active_accounts)),
        ...
        if "gv" in active_accounts or "kevin" in active_accounts:
            ...
            scenarios.append({
                "type": "bot", "tier": tier, "id": "clabe_invalida", "account": acct,
                "messages": ["necesito 10000 para IAS neto", "123456789012345678"],
                "expected": ["inválida", "incorrecta", "error", "CLABE"],
            })
        scenarios.append({"type": "chat", "tier": tier, "id": "post_tarjetas", "turns": [
...
async def run_scenario(clients: dict, chat_entities: dict, scenario: dict,
    ...
    if not client:
        return {"test_id": test_id, "passed": False, "detail": f"cuenta {account} no disponible"}
                ...
                try:
                    ...
                    if asset_spec:
                        ...
                        label = (f"[xlsx {fname}]" if is_doc else
                                 f"[png cuadro sem{meta.get('semana','?')} {meta.get('clientes','?')} clientes"
                    ...
                    try:
                        ...
                        buf = io.BytesIO(data)
                        buf.name = fname
                        await client.send_file(target, buf, caption=cap, force_document=is_doc,
                                               **({
...
def _push_episode_to_relay(episode_id: int, results: list, stats: dict, round_num: int):
    ...
    entry = {
        "episode_id": episode_id, "round_num": round_num,
        "ts": datetime.utcnow().isoformat() + 'Z',
        "score": round(stats.get('score', 0), 1),
        "passed": stats.get('passed', 0), "total": stats.get('total', 0),
        "results": [{"id": r.get("test_id", "?"), "passed": r.get("passed", False),
                     "detail": (r.get("detail") or "")[:120]} for r in results],
    }
    try:
        ...
        subprocess.run(['git', 'add', 'relay/episode-results.jsonl'], cwd=REPO_ROOT, capture_output=True, timeout=10)
        subprocess.run(['git', 'commit', '--no-gpg-sign', '-m', f'ep#{episode_id} r{round_num} {stats.get("score",0):.1f}%'],
                       cwd=REPO_ROOT, capture_output=True, timeout=15)
        subprocess.run(['git', 'push', 'origin', branch], cwd=REPO_ROOT, capture_output=True, timeout=30)
    ...
    while rounds == 0 or round_num < rounds:
        ...
        if episode_id and round_results:
            try:
                ...
                print(f"\n Score ronda #{round_num}: {stats['score']:.1f}% ({stats['passed']}/{stats['total']})")
                if round_num % ROUNDS_PER_REPORT == 0:
...
def main():
    ...
    parser.add_argument("--rounds", type=int, default=0, help="Numero de rondas (0=infinito)")
    parser.add_argument("--tier", type=int, default=0, choices=[0, 1, 2, 3, 4], help="Forzar tier (0=automatico)")
    parser.add_argument("--dry-run", action="store_true", help="No envia mensajes, solo simula")
