...
async def run_scenario(clients: dict, scenario: dict, chat_id: int,
                       episode_id: int, dry_run: bool = False) -> dict:
    """
    ...
    if not dry_run:
        # Enviar primer mensaje y esperar respuesta
        await client.send_message(chat_id, messages[0])
        bot_response = await wait_for_bot_response(client, chat_id, BOT_RESPONSE_TIMEOUT)

        # Enviar mensajes siguientes (CLABE, confirmar/cancelar, etc.)
        for i, msg in enumerate(messages[1:], 1):
            await asyncio.sleep(DELAY_BETWEEN_MESSAGES)
            print(f"  [{account.upper()}] → {msg[:60]}")
            await client.send_message(chat_id, msg)
            # Solo esperar respuesta del bot en el último mensaje
            if i == len(messages) - 1:
    ...
    # Evaluar si el bot respondió correctamente
    if bot_response is None:
        ...
        detail = (f"OK — '{bot_response[:80]}'" if passed
                  else f"respuesta sin keywords esperadas {expected[:3]}: '{bot_response[:80]}'")
...
async def run_engine(rounds: int = 0, force_tier: int = 0, dry_run: bool = False):
    ...
    while rounds == 0 or round_num < rounds:
        ...
        for scenario in scenarios:
            result = await run_scenario(clients, scenario, chat_id, episode_id, dry_run)
