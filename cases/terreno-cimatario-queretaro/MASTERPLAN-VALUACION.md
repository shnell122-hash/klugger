...
- Dataset previo "1000" = **824 sintéticos (`example.com`) + 176 nav-links vacíos = 0 reales usables**.
...
| 5 | Enriquecer metadata completa (detalle) | **TÚ** | red en vivo + re-clean autoritativo | — | 3 | geo+price+addr por listing | 🟡 corriendo (bg) |
| 6 | Re-clean con offer_price autoritativo | **TÚ** | rescata mis-parseados → maximiza a ~1000 | — | 5 | conteo limpio final | ⬜ |
| 7 | (Si IP_ROYAL) Inmuebles24/Vivanuncios vía IPRoyal | **TÚ** | proxy = secreto en runtime, sensible | ∥ con 5 | IP_ROYAL | +fuentes, 0 fakes | ⛔ bloqueado (key) |
| 8 | Transform → `terrenos_full.json` (números+coords) | Sonnet | impl. mecánica con spec | Ola A | 6 | schema válido, build ok | ⬜ |
