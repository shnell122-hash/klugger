...
| K2 | 2 | **Organismos D** (mapa, ficha, AVM, verificación, shortlist, copiloto) | Opus (AVM/verif/seguridad) + Sonnet | smoke tests de dominio | ✅ **completo** — AVM (rango+factores) · verificación itemizada · card↔pin map-first · historial precio · shortlist (77d6e93) + galería/lightbox (D2) · floor-plan clicable (D3) · copiloto agente (D8) · data-room cifrado (D9) (fcb1b21, deploy). Falta solo el mapa **Mapbox real** (hoy mock) y tour 3D real (embed) — requieren token/asset. |
| K3 | 3 | **Arte en movimiento F** (GSAP/ScrollTrigger/Lottie/video) cableando assets de R2 | Sonnet + operador (arte) | scroll-story fluido 60fps; reduced-motion | ✅ **hecho + rehecho con video real** — **Hero = video low-poly de german (Grok Imagine): el zorro recorre CDMX, scrubbeado por scroll (F1+F2+F5 juntos)** · line-draw imagotipo atado a scroll (F6) · mascota-estados (F4) · marquee (F7) · reveal-on-scroll (F8) (commit c991b9f, deploy). Se descartó el parallax+scroll-story falsos (SVG sobre imagen). **Pendientes de asset del operador:** Lottie de mascota (F4 rico), atlas isométrico (F10), transición 2D↔low-poly (F11). |
