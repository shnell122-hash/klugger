...
| K3 | 3 | **Arte en movimiento F** (GSAP/ScrollTrigger/Lottie/video) cableando assets de R2 | Sonnet + operador (arte) | scroll-story fluido 60fps; reduced-motion | ✅ **hecho** — hero parallax (F1/F3) · zorro recorre el mapa pin+scrub (F2) · line-draw imagotipo stroke-dashoffset (F6) · mascota-estados (F4) · marquee colonias (F7) · reveal-on-scroll (F8) (commit 04df46d, deploy). **Pendientes de asset del operador:** video-loop Veo (F5), Lottie de mascota (F4 rico), atlas isométrico (F10), transición 2D↔low-poly (F11). |
